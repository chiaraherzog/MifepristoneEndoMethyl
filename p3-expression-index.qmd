...
# S4C --------
out <- volcano$data
write.table(out, file = 'SourceData/s4c.txt', quote = F, sep = '\t', row.names = F)

# S4D--------
out <- mat |> 
  as.data.frame() |> tibble::rownames_to_column('gene')
write.table(out, file = 'SourceData/s4d.txt', quote = F, sep = '\t', row.names = F)
```
...
```{r}
# S4B --------
out <- data_exprtest |> dplyr::select(set, type, index) |> 
  dplyr::rename(TCGA_EC_expression = index) |> tibble::rownames_to_column('barcode')
