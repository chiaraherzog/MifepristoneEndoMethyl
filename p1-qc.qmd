...
age <- readxl::read_xlsx(file.path(db_path, "data/mifepristone/endometrium/pheno/DNA endometrium_220311_annotated.xlsx"),
  ...
  dplyr::mutate(mens_phase = dplyr::case_when(mens_phase == '?' ~ 'unknown',
                                       mens_phase %in% c("no menstruation") ~ "no menstruation",
                                       mens_phase == "not regular menstruation, 5 weeks since last" ~ 'irregular menstruation',
...
# Adding PRBS in review:

prbs <- eutopssrc::prbs(beta_merged)
pheno$prbs_all <- prbs$PRBS
pheno$prbs_hypoM <- prbs$PRBS_hypoM
...
data <- data |> dplyr::mutate(mens_phase = factor(mens_phase, levels = c("early luteal",
                                                                         "mid luteal", 
                                                                         "late luteal",
                                                                         "ongoing menstruation",
                                                                         "irregular menstruation",
...
t <- tbl_merge(list(b, a), tab_spanner = c("Vitamin B", "Mifepristone")) |> as_gt()
...
t |> text_replace(
    ...
    locations = cells_body(columns = everything(),
                           rows = everything())
  ) |> opt_row_striping() |> opt_stylize(style = 3,color = 'blue') 
```
...
cols <- c("unknown" = 'grey70',
          'no menstruation' = '#A28A2D',
          'irregular menstruation' = 'magenta',
          'early luteal' = '#001959',
          'mid luteal' = '#2C635B',
          'late luteal' = '#FDB1AB',
...
# B --------
out <- pcTile$data
write.table(out, file = 'SourceData/1b.txt', quote = F, sep = '\t', row.names = F)

# C --------
out <- ic$data |> dplyr::select(PC1, PC2, ic)
write.table(out, file = 'SourceData/1c.txt', quote = F, sep = '\t', row.names = F)

# D --------
out <- time$data |> dplyr::select(PC1, PC2, trt, time)
write.table(out, file = 'SourceData/1d.txt', quote = F, sep = '\t', row.names = F)

# E --------
out <- p$data |> dplyr::select(PC1, PC2, mens_phase)
write.table(out, file = 'SourceData/1e.txt', quote = F, sep = '\t', row.names = F)
```
...
```{r}
out <- usthickness$data |> dplyr::select(us, mens_phase)
write.table(out, file = 'SourceData/s2.txt', quote = F, sep = '\t', row.names = F)
```
...
# A --------
out <- snps$data |> 
  # dplyr::mutate(anon_rsid = as.integer(factor(rs))) |> 
  dplyr::mutate(anon_rsid = sample.int(n_distinct(rs))[match(rs, unique(rs))]) |> 
  dplyr::group_by(participant_id) |> 
  dplyr::mutate(id = dplyr::cur_group_id()) |> 
  dplyr::ungroup() |> 
  dplyr::select(id, anon_rsid, post, pre)
write.table(out, file = 'SourceData/s3a.txt', quote = F, sep = '\t', row.names = F)

# B --------
out <- assayIntensity$data |> dplyr::select(mMed, uMed)
write.table(out, file = 'SourceData/s3b.txt', quote = F, sep = '\t', row.names = F)

# C --------
out <- betaDist$data |> dplyr::group_by(basename) |> 
  dplyr::mutate(sample = dplyr::cur_group_id()) |>
  dplyr::ungroup() |> 
  dplyr::select(sample, x, y, ic) |> 
  dplyr::rename(beta = x, density = y)
write.table(out, file = 'SourceData/s3c.txt', quote = F, sep = '\t', row.names = F)
```
...
```{r}
# 1 --------
out <- celltypesPrePost$data |>
  dplyr::group_by(participant_id) |> 
  dplyr::mutate(participant = dplyr::cur_group_id()) |>
  dplyr::ungroup() |> 
  dplyr::select(participant, celltype, cell_prop, time)
write.table(out, file = 'SourceData/3_panel1.txt', quote = F, sep = '\t', row.names = F)

# 2 --------
out <- celltypesDelta$data |> 
  dplyr::select(-c(participant_id, pre, post))
write.table(out, file = 'SourceData/3_panel2.txt', quote = F, sep = '\t', row.names = F)
```
